...
class RequestOperations {
    ...
    public function alertPortersAboutNewRequest(RequestsDB $request_) {
		...
		foreach ($porters as $porter) {
            $msg0  = 'Hey ' . $porter->getFirstName() . ',<br><br>';
            $msg1  = 'A new job is now available in ' . $request_->getRepCity()->getName() . '. Check out the details by following the link below. Please do not reply to this email.<br>';
            $msg2  = '<a href="http://www.summonporter.ca' . $this->router->generate('internal_services_requests', ['requestToken_' => $request_->getIdToken(), 'porterToken_' => $porter->getIdToken()], UrlGeneratorInterface::ABSOLUTE_PATH) . '">Job page.</a>';
            $msg3 .= '<br><br>The Porter Alert Bot';
            $hMsg  = $msg0 . $msg1 . $msg2 . $msg3;

            $msg0  = 'Hey ' . $porter->getFirstName() . ',\r\n';
            $msg1  = ' new job is now available in ' . $request_->getRepCity()->getName() . '. Check out the details by following the link below. Please do not reply to this email.\r\n';
            $msg2  = 'http://www.summonporter.ca' . $this->router->generate('internal_services_requests', ['requestToken_' => $request_->getIdToken(), 'porterToken_' => $porter->getIdToken()], UrlGeneratorInterface::ABSOLUTE_PATH);
    ...
    public function alertPorterAboutAssignment(RequestsDB $request_, PortersDB $porter_) {
        ...
        $msg1  = 'You have been assigned to a job in ' . $request_->getRepCity()->getName() . '. Check out the details by following the link below. <b>Please keep this email!</b> Please do not reply to this email.<br>';
        ...
        $msg1  = 'You have been assigned to a job in ' . $request_->getRepCity()->getName() . '. Check out the details by following the link below. Please keep this email! Please do not reply to this email.\r\n';
